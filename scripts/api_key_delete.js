		...
		act.on_message(function (msg, reply, info) {
				...
				if(key) {
						...
						balanced.api_key.create().then(function (new_key) {
								act.msg(info.nick, 'Here is your new api key, please do not publically share it again: '+new_key.secret);
										...
										k.all().then(function(keys) {
												for(var a=0; a<keys.length; a++) {
														if(keys[a].secret) { // secret only comes back for the current api key
																keys[a].unstore();
																reply(true, 'your api key '+keys[a].secret+' has been deleted for your protection, a new api has been sent to you in a private message, please do not shared your marketplace secret key in irc as this channel is publically logged');
														}
												}
										});
								});
